wage_diff_by_variables <- function(df, factor_var, dummy_var, factor_lab, dummy_lab){
  ...
  # df <-  data
  # factor_var <- "f_room_type"
  # dummy_var  <- "d_familykidfriendly"
  # factor_lab <- "f_room_type"
  # dummy_lab <- "d_familykidfriendly"
  ...
  stats <- df %>%
    ...
    dplyr::summarize(Mean = mean(wages_per_hour, na.rm=TRUE),
                     se = sd(wages_per_hour)/sqrt(n()))
