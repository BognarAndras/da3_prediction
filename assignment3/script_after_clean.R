...
sum(is.na(data$d1_rapid_sales_growth))
table(data$d2_avg_conseq_fast_sales_growth)

table(data.table(data)[is.na(d2_avg_conseq_fast_sales_growth) , new])
sum(is.na(data$d1_sales))
...
data.table(melted_cormat)[Var1 == 'd1_fast_sales_growth' | Var2 == 'd1_fast_sales_growth' ][order(value)][1:30]
# obviously big corr with sales related measures 
#  ln_sales ind female share_eq_bs  extra_inc  extra_profit_loss

#  age age2  
# b2_sales_growth b1_sales_growth
...
summary(data$b1_sales)

g1 <- ggplot( data , aes(x = age, y = d1_fast_sales_growth)) +
  geom_point(color='red',size=2,alpha=0.6) +
  geom_smooth(method="loess" , formula = y ~ x , se = F)+
  labs(x = "age", y = "Growth rate compared to last year") +
  scale_x_continuous(breaks = seq(0 , 30 , 5 )) +
  theme_bw() +
  theme(axis.title.y = element_text(size = 10) )

summary(data$b2_sales_growth)
g2 <- ggplot( data , aes(x = b2_sales, y = b1_sales)) +
  geom_point(color='red',size=2,alpha=0.6) +
  geom_smooth(method="loess" , formula = y ~ x , se = F)+
  labs(x = "age", y = "Growth rate compared to last year") +
  theme_bw() +
...
chck_sp(data$age , "age")
# linear / quad
chck_sp(data$ln_sales , "ln_sales")
# linear / quad
chck_sp(data$ind , "ind")
# defintetely quad
chck_sp(data$female , "female")
...
ggplot(data, aes(x = factor(n_number_of_reviews), y = price,
                 fill = factor(n_host_is_superhost), color=factor(n_host_is_superhost))) +
...
feols(d1_fast_sales_growth ~ d1_sales , data = data , vcov = "hetero")

# Linear probability model
ols_modelx2 <- feols(formula(paste0("d1_fast_sales_growth ~", paste0(X2, collapse = " + "))),
                     data = data , vcov = 'hetero')
summary(ols_modelx2)

ols_modelx0 <- feols(formula(paste0("d1_fast_sales_growth ~", paste0(X0, collapse = " + "))),
                     data = data , vcov = 'hetero')
summary(ols_modelx0)

# Logit model
glm_modelx2 <- glm(formula(paste0("d1_fast_sales_growth ~", paste0(X2, collapse = " + "))),
                   data = data, family = "binomial")
summary(glm_modelx2)

glm_modelx0 <- glm(formula(paste0("d1_fast_sales_growth ~", paste0(X0, collapse = " + "))),
                   data = data, family = "binomial")
summary(glm_modelx0)
...
mx0 <- margins(glm_modelx0, vce = "none")
...
cm_object1 <- confusionMatrix(logit_class_prediction, data_holdout$d1_fast_sales_growth, positive = "fast_growth")
cm_object1
...
cm_object1b <- confusionMatrix(holdout_prediction,data_holdout$d1_fast_sales_growth)
...
holdout_prediction <-
  ...
  factor(levels = c("no_fast_growth", "fast_growth"))
cm_object2 <- confusionMatrix(holdout_prediction,data_holdout$d1_fast_sales_growth)
...
#  a) models
#  b) Folds
cv_fold <-  logit_models[["X2"]]$pred %>% filter(Resample == "Fold5")
logit_models[["X2"]]$
  is.numeric(cv_fold$fast_growth  )
roc_obj <- roc(cv_fold$obs, cv_fold$fast_growth, quiet = TRUE)
...
levels(data_for_graph$d1_fast_sales_growth) <- list("stagnate" = "no_fast_growth", "growth" = "fast_growth")

# First a simple CART (with pre-set cp and minbucket)
set.seed(13505)
rf_for_graph <-
  rpart(
    formula = d1_fast_sales_growth ~ sales_mil + profit_loss_year+ foreign_management,
    data = data_for_graph,
